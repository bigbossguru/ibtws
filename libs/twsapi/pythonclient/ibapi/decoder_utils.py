...
def decodeOrder(orderId: int, contractProto: ContractProto, orderProto: OrderProto) -> Order:
    ...
    if orderProto.HasField("hedgeType") and orderProto.HasField("hedgeParam") and orderProto.hedgeType:
        order.hedgeParam = orderProto.hedgeParam
    if orderProto.HasField("hedgeType") and orderProto.HasField("hedgeMaxSize"):
        order.hedgeMaxSize = orderProto.hedgeMaxSize
    ...
    if orderProto.HasField("postOnly"):
        order.postOnly = orderProto.postOnly
    if orderProto.HasField("allowPreOpen"):
        order.allowPreOpen = orderProto.allowPreOpen
    if orderProto.HasField("ignoreOpenAuction"):
        order.ignoreOpenAuction = orderProto.ignoreOpenAuction
    if orderProto.HasField("deactivate"):
        order.deactivate = orderProto.deactivate
    if orderProto.HasField("activeStartTime"):
        order.activeStartTime = orderProto.activeStartTime
    if orderProto.HasField("activeStopTime"):
        order.activeStopTime = orderProto.activeStopTime
    if orderProto.HasField("seekPriceImprovement"):
        order.seekPriceImprovement = orderProto.seekPriceImprovement
    if orderProto.HasField("whatIfType"):
        order.whatIfType = orderProto.whatIfType
...
def setConditionFields(orderConditionProto: OrderConditionProto, orderCondition: OrderCondition):
    if orderConditionProto.HasField("isConjunctionConnection"):
        orderCondition.isConjunctionConnection = orderConditionProto.isConjunctionConnection
...
def setContractConditionFields(orderConditionProto: OrderConditionProto, contractCondition: ContractCondition):
    ...
    if orderConditionProto.HasField("exchange"):
        contractCondition.exchange = orderConditionProto.exchange
...
def createExecutionCondition(orderConditionProto: OrderConditionProto) -> ExecutionCondition:
    ...
    if orderConditionProto.HasField("exchange"):
        executionCondition.exchange = orderConditionProto.exchange
...
def decodeOrderState(orderStateProto: OrderStateProto) -> OrderState:
    ...
    if orderStateProto.HasField("initMarginBefore"):
        orderState.initMarginBefore = decimalMaxString(orderStateProto.initMarginBefore)
    if orderStateProto.HasField("maintMarginBefore"):
        orderState.maintMarginBefore = decimalMaxString(orderStateProto.maintMarginBefore)
    if orderStateProto.HasField("equityWithLoanBefore"):
        orderState.equityWithLoanBefore = decimalMaxString(orderStateProto.equityWithLoanBefore)
    ...
    if orderStateProto.HasField("maintMarginChange"):
        orderState.maintMarginChange = decimalMaxString(orderStateProto.maintMarginChange)
    if orderStateProto.HasField("equityWithLoanChange"):
        orderState.equityWithLoanChange = decimalMaxString(orderStateProto.equityWithLoanChange)
    ...
    if orderStateProto.HasField("equityWithLoanAfter"):
        orderState.equityWithLoanAfter = decimalMaxString(orderStateProto.equityWithLoanAfter)
    ...
    if orderStateProto.HasField("initMarginBeforeOutsideRTH"):
        orderState.initMarginBeforeOutsideRTH = orderStateProto.initMarginBeforeOutsideRTH
    if orderStateProto.HasField("maintMarginBeforeOutsideRTH"):
        orderState.maintMarginBeforeOutsideRTH = orderStateProto.maintMarginBeforeOutsideRTH
    if orderStateProto.HasField("equityWithLoanBeforeOutsideRTH"):
        orderState.equityWithLoanBeforeOutsideRTH = orderStateProto.equityWithLoanBeforeOutsideRTH
    if orderStateProto.HasField("initMarginChangeOutsideRTH"):
        orderState.initMarginChangeOutsideRTH = orderStateProto.initMarginChangeOutsideRTH
    if orderStateProto.HasField("maintMarginChangeOutsideRTH"):
        orderState.maintMarginChangeOutsideRTH = orderStateProto.maintMarginChangeOutsideRTH
    if orderStateProto.HasField("equityWithLoanChangeOutsideRTH"):
        orderState.equityWithLoanChangeOutsideRTH = orderStateProto.equityWithLoanChangeOutsideRTH
    ...
    if orderStateProto.HasField("maintMarginAfterOutsideRTH"):
        orderState.maintMarginAfterOutsideRTH = orderStateProto.maintMarginAfterOutsideRTH
    if orderStateProto.HasField("equityWithLoanAfterOutsideRTH"):
        orderState.equityWithLoanAfterOutsideRTH = orderStateProto.equityWithLoanAfterOutsideRTH
    ...
    if orderAllocationProtoList:
        for orderAllocationProto in orderAllocationProtoList:
            ...
            if orderAllocationProto.HasField("positionDesired"):
                orderAllocation.positionDesired = Decimal(orderAllocationProto.positionDesired)
            if orderAllocationProto.HasField("positionAfter"):
                orderAllocation.positionAfter = Decimal(orderAllocationProto.positionAfter)
            if orderAllocationProto.HasField("desiredAllocQty"):
                orderAllocation.desiredAllocQty = Decimal(orderAllocationProto.desiredAllocQty)
            if orderAllocationProto.HasField("allowedAllocQty"):
                orderAllocation.allowedAllocQty = Decimal(orderAllocationProto.allowedAllocQty)
...
@staticmethod
def decodeContractDetails(
    contractProto: ContractProto, contractDetailsProto: ContractDetailsProto, isBond: bool
) -> ContractDetails:
    ...
    if contractDetailsProto.HasField("marketName"):
        contractDetails.marketName = contractDetailsProto.marketName
    if contractDetailsProto.HasField("minTick"):
        contractDetails.minTick = float(contractDetailsProto.minTick)
    if contractDetailsProto.HasField("priceMagnifier"):
        contractDetails.priceMagnifier = contractDetailsProto.priceMagnifier
    if contractDetailsProto.HasField("orderTypes"):
        contractDetails.orderTypes = contractDetailsProto.orderTypes
    if contractDetailsProto.HasField("validExchanges"):
        contractDetails.validExchanges = contractDetailsProto.validExchanges
    if contractDetailsProto.HasField("underConId"):
        contractDetails.underConId = contractDetailsProto.underConId
    if contractDetailsProto.HasField("longName"):
        contractDetails.longName = contractDetailsProto.longName
    if contractDetailsProto.HasField("contractMonth"):
        contractDetails.contractMonth = contractDetailsProto.contractMonth
    if contractDetailsProto.HasField("industry"):
        contractDetails.industry = contractDetailsProto.industry
    if contractDetailsProto.HasField("category"):
        contractDetails.category = contractDetailsProto.category
    if contractDetailsProto.HasField("subcategory"):
        contractDetails.subcategory = contractDetailsProto.subcategory
    if contractDetailsProto.HasField("timeZoneId"):
        contractDetails.timeZoneId = contractDetailsProto.timeZoneId
    if contractDetailsProto.HasField("tradingHours"):
        contractDetails.tradingHours = contractDetailsProto.tradingHours
    if contractDetailsProto.HasField("liquidHours"):
        contractDetails.liquidHours = contractDetailsProto.liquidHours
    if contractDetailsProto.HasField("evRule"):
        contractDetails.evRule = contractDetailsProto.evRule
    if contractDetailsProto.HasField("evMultiplier"):
        contractDetails.evMultiplier = contractDetailsProto.evMultiplier

    contractDetails.secIdList = decodeTagValueList(contractDetailsProto.secIdList)

    if contractDetailsProto.HasField("aggGroup"):
        contractDetails.aggGroup = contractDetailsProto.aggGroup
    if contractDetailsProto.HasField("underSymbol"):
        contractDetails.underSymbol = contractDetailsProto.underSymbol
    if contractDetailsProto.HasField("underSecType"):
        contractDetails.underSecType = contractDetailsProto.underSecType
    if contractDetailsProto.HasField("marketRuleIds"):
        contractDetails.marketRuleIds = contractDetailsProto.marketRuleIds
    if contractDetailsProto.HasField("realExpirationDate"):
        contractDetails.realExpirationDate = contractDetailsProto.realExpirationDate
    if contractDetailsProto.HasField("stockType"):
        contractDetails.stockType = contractDetailsProto.stockType
    if contractDetailsProto.HasField("minSize"):
        contractDetails.minSize = Decimal(contractDetailsProto.minSize)
    if contractDetailsProto.HasField("sizeIncrement"):
        contractDetails.sizeIncrement = Decimal(contractDetailsProto.sizeIncrement)
    if contractDetailsProto.HasField("suggestedSizeIncrement"):
        contractDetails.suggestedSizeIncrement = Decimal(contractDetailsProto.suggestedSizeIncrement)
    if contractDetailsProto.HasField("minAlgoSize"):
        contractDetails.minAlgoSize = Decimal(contractDetailsProto.minAlgoSize)
    if contractDetailsProto.HasField("lastPricePrecision"):
        contractDetails.lastPricePrecision = Decimal(contractDetailsProto.lastPricePrecision)
    if contractDetailsProto.HasField("lastSizePrecision"):
        contractDetails.lastSizePrecision = Decimal(contractDetailsProto.lastSizePrecision)

    setLastTradeDate(contract.lastTradeDateOrContractMonth, contractDetails, isBond)

    if contractDetailsProto.HasField("cusip"):
        contractDetails.cusip = contractDetailsProto.cusip
    if contractDetailsProto.HasField("ratings"):
        contractDetails.ratings = contractDetailsProto.ratings
    if contractDetailsProto.HasField("descAppend"):
        contractDetails.descAppend = contractDetailsProto.descAppend
    if contractDetailsProto.HasField("bondType"):
        contractDetails.bondType = contractDetailsProto.bondType
    if contractDetailsProto.HasField("coupon"):
        contractDetails.coupon = contractDetailsProto.coupon
    if contractDetailsProto.HasField("couponType"):
        contractDetails.couponType = contractDetailsProto.couponType
    if contractDetailsProto.HasField("callable"):
        contractDetails.callable = contractDetailsProto.callable
    if contractDetailsProto.HasField("puttable"):
        contractDetails.putable = contractDetailsProto.puttable
    if contractDetailsProto.HasField("convertible"):
        contractDetails.convertible = contractDetailsProto.convertible
    if contractDetailsProto.HasField("issueDate"):
        contractDetails.issueDate = contractDetailsProto.issueDate
    if contractDetailsProto.HasField("nextOptionDate"):
        contractDetails.nextOptionDate = contractDetailsProto.nextOptionDate
    if contractDetailsProto.HasField("nextOptionType"):
        contractDetails.nextOptionType = contractDetailsProto.nextOptionType
    if contractDetailsProto.HasField("nextOptionPartial"):
        contractDetails.nextOptionPartial = contractDetailsProto.nextOptionPartial
    if contractDetailsProto.HasField("bondNotes"):
        contractDetails.notes = contractDetailsProto.bondNotes

    if contractDetailsProto.HasField("fundName"):
        contractDetails.fundName = contractDetailsProto.fundName
    if contractDetailsProto.HasField("fundFamily"):
        contractDetails.fundFamily = contractDetailsProto.fundFamily
    if contractDetailsProto.HasField("fundType"):
        contractDetails.fundType = contractDetailsProto.fundType
    if contractDetailsProto.HasField("fundFrontLoad"):
        contractDetails.fundFrontLoad = contractDetailsProto.fundFrontLoad
    if contractDetailsProto.HasField("fundBackLoad"):
        contractDetails.fundBackLoad = contractDetailsProto.fundBackLoad
    if contractDetailsProto.HasField("fundBackLoadTimeInterval"):
        contractDetails.fundBackLoadTimeInterval = contractDetailsProto.fundBackLoadTimeInterval
    if contractDetailsProto.HasField("fundManagementFee"):
        contractDetails.fundManagementFee = contractDetailsProto.fundManagementFee
    if contractDetailsProto.HasField("fundClosed"):
        contractDetails.fundClosed = contractDetailsProto.fundClosed
    if contractDetailsProto.HasField("fundClosedForNewInvestors"):
        contractDetails.fundClosedForNewInvestors = contractDetailsProto.fundClosedForNewInvestors
    if contractDetailsProto.HasField("fundClosedForNewMoney"):
        contractDetails.fundClosedForNewMoney = contractDetailsProto.fundClosedForNewMoney
    if contractDetailsProto.HasField("fundNotifyAmount"):
        contractDetails.fundNotifyAmount = contractDetailsProto.fundNotifyAmount
    if contractDetailsProto.HasField("fundMinimumInitialPurchase"):
        contractDetails.fundMinimumInitialPurchase = contractDetailsProto.fundMinimumInitialPurchase
    if contractDetailsProto.HasField("fundMinimumSubsequentPurchase"):
        contractDetails.fundSubsequentMinimumPurchase = contractDetailsProto.fundMinimumSubsequentPurchase
    if contractDetailsProto.HasField("fundBlueSkyStates"):
        contractDetails.fundBlueSkyStates = contractDetailsProto.fundBlueSkyStates
    if contractDetailsProto.HasField("fundBlueSkyTerritories"):
        contractDetails.fundBlueSkyTerritories = contractDetailsProto.fundBlueSkyTerritories

    if contractDetailsProto.HasField("fundDistributionPolicyIndicator"):
        contractDetails.fundDistributionPolicyIndicator = getEnumTypeFromString(
            FundDistributionPolicyIndicator, contractDetailsProto.fundDistributionPolicyIndicator
        )
    if contractDetailsProto.HasField("fundAssetType"):
        contractDetails.fundAssetType = getEnumTypeFromString(FundAssetType, contractDetailsProto.fundAssetType)
    ...
    if contractDetailsProto.HasField("eventContract1"):
        contractDetails.eventContract1 = contractDetailsProto.eventContract1
    if contractDetailsProto.HasField("eventContractDescription1"):
        contractDetails.eventContractDescription1 = contractDetailsProto.eventContractDescription1
    if contractDetailsProto.HasField("eventContractDescription2"):
        contractDetails.eventContractDescription2 = contractDetailsProto.eventContractDescription2
...
def decodeHistoricalTick(historicalTickProto: HistoricalTickProto) -> HistoricalTick:
    ...
    if historicalTickProto.HasField("time"):
        historicalTick.time = historicalTickProto.time
    if historicalTickProto.HasField("price"):
        historicalTick.price = historicalTickProto.price
    if historicalTickProto.HasField("size"):
        historicalTick.size = Decimal(historicalTickProto.size)
...
def decodeHistoricalTickBidAsk(historicalTickBidAskProto: HistoricalTickBidAskProto) -> HistoricalTickBidAsk:
    ...
    if historicalTickBidAskProto.HasField("time"):
        historicalTickBidAsk.time = historicalTickBidAskProto.time
    ...
    if historicalTickBidAskProto.HasField("tickAttribBidAsk"):
        ...
        if tickAttribBidAskProto.HasField("bidPastLow"):
            tickAttribBidAsk.bidPastLow = tickAttribBidAskProto.bidPastLow
        if tickAttribBidAskProto.HasField("askPastHigh"):
            tickAttribBidAsk.askPastHigh = tickAttribBidAskProto.askPastHigh
    historicalTickBidAsk.tickAttribBidAsk = tickAttribBidAsk

    if historicalTickBidAskProto.HasField("priceBid"):
        historicalTickBidAsk.priceBid = historicalTickBidAskProto.priceBid
    if historicalTickBidAskProto.HasField("priceAsk"):
        historicalTickBidAsk.priceAsk = historicalTickBidAskProto.priceAsk
    if historicalTickBidAskProto.HasField("sizeBid"):
        historicalTickBidAsk.sizeBid = Decimal(historicalTickBidAskProto.sizeBid)
    if historicalTickBidAskProto.HasField("sizeAsk"):
        historicalTickBidAsk.sizeAsk = Decimal(historicalTickBidAskProto.sizeAsk)
...
def decodeHistoricalTickLast(historicalTickLastProto: HistoricalTickLastProto) -> HistoricalTickLast:
    ...
    if historicalTickLastProto.HasField("time"):
        historicalTickLast.time = historicalTickLastProto.time

    tickAttribLast = TickAttribLast()
    if historicalTickLastProto.HasField("tickAttribLast"):
        tickAttribLastProto = historicalTickLastProto.tickAttribLast
        if tickAttribLastProto.HasField("pastLimit"):
            tickAttribLast.pastLimit = tickAttribLastProto.pastLimit
        if tickAttribLastProto.HasField("unreported"):
            tickAttribLast.unreported = tickAttribLastProto.unreported
    historicalTickLast.tickAttribLast = tickAttribLast

    if historicalTickLastProto.HasField("price"):
        historicalTickLast.price = historicalTickLastProto.price
    if historicalTickLastProto.HasField("size"):
        historicalTickLast.size = Decimal(historicalTickLastProto.size)
    if historicalTickLastProto.HasField("exchange"):
        historicalTickLast.exchange = historicalTickLastProto.exchange
    if historicalTickLastProto.HasField("specialConditions"):
        historicalTickLast.specialConditions = historicalTickLastProto.specialConditions
...
def decodeHistogramDataEntry(histogramDataEntryProto: HistogramDataEntryProto) -> HistogramData:
    ...
    if histogramDataEntryProto.HasField("size"):
        histogramData.size = Decimal(histogramDataEntryProto.size)
...
def decodeHistoricalDataBar(historicalDataBarProto: HistoricalDataBarProto) -> BarData:
    ...
    if historicalDataBarProto.HasField("date"):
        bar.date = historicalDataBarProto.date
    if historicalDataBarProto.HasField("open"):
        bar.open = historicalDataBarProto.open
    if historicalDataBarProto.HasField("high"):
        bar.high = historicalDataBarProto.high
    if historicalDataBarProto.HasField("low"):
        bar.low = historicalDataBarProto.low
    if historicalDataBarProto.HasField("close"):
        bar.close = historicalDataBarProto.close
    if historicalDataBarProto.HasField("volume"):
        bar.volume = Decimal(historicalDataBarProto.volume)
    if historicalDataBarProto.HasField("WAP"):
        bar.wap = Decimal(historicalDataBarProto.WAP)
    if historicalDataBarProto.HasField("barCount"):
        bar.barCount = historicalDataBarProto.barCount
    return bar
...
def decodeFamilyCode(familyCodeProto: FamilyCodeProto) -> FamilyCode:
    ...
    if familyCodeProto and familyCodeProto.HasField("accountId"):
        familyCode.accountID = familyCodeProto.accountId
    if familyCodeProto and familyCodeProto.HasField("familyCode"):
        familyCode.familyCodeStr = familyCodeProto.familyCode
    return familyCode
...
def decodeSmartComponents(smartComponentsProto: SmartComponentsProto) -> SmartComponentMap:
    ...
    if smartComponentsProto and smartComponentsProto.smartComponents:
        for smartComponentProto in smartComponentsProto.smartComponents:
            ...
            exchangeLetter = (
                smartComponentProto.exchangeLetter if smartComponentProto.HasField("exchangeLetter") else " "
            )
...
def decodePriceIncrement(priceIncrementProto: PriceIncrementProto) -> PriceIncrement:
    ...
    if priceIncrementProto and priceIncrementProto.HasField("lowEdge"):
        priceIncrement.lowEdge = priceIncrementProto.lowEdge
    if priceIncrementProto and priceIncrementProto.HasField("increment"):
        priceIncrement.increment = priceIncrementProto.increment
    return priceIncrement


@staticmethod
def decodeDepthMarketDataDescription(
    depthMarketDataDescriptionProto: DepthMarketDataDescriptionProto,
) -> DepthMktDataDescription:
    description = DepthMktDataDescription()
    if depthMarketDataDescriptionProto.HasField("exchange"):
        description.exchange = depthMarketDataDescriptionProto.exchange
    if depthMarketDataDescriptionProto.HasField("secType"):
        description.secType = depthMarketDataDescriptionProto.secType
    if depthMarketDataDescriptionProto.HasField("listingExch"):
        description.listingExch = depthMarketDataDescriptionProto.listingExch
    if depthMarketDataDescriptionProto.HasField("serviceDataType"):
        description.serviceDataType = depthMarketDataDescriptionProto.serviceDataType
    if depthMarketDataDescriptionProto.HasField("aggGroup"):
        description.aggGroup = depthMarketDataDescriptionProto.aggGroup
    return description
