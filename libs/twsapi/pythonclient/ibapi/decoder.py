...
from ibapi.decoder_utils import (
    ...
    decodeExecution,
    decodeOrderState,
    decodeContractDetails,
    setLastTradeDate,
)
from ibapi.decoder_utils import (
    decodeHistoricalDataBar,
...
from ibapi.decoder_utils import (
    ...
    decodeFamilyCode,
    decodeSmartComponents,
    decodePriceIncrement,
    decodeDepthMarketDataDescription,
)
...
class Decoder(Object):
    ...
    def processContractDataMsg(self, fields):
        ...
        if self.serverVersion >= MIN_SERVER_VER_FUND_DATA_FIELDS and contract.contract.secType == "FUND":
    ...
    def processTickOptionComputationMsg(self, fields):
        ...
        if version >= 6 or tickTypeInt == TickTypeEnum.MODEL_OPTION or tickTypeInt == TickTypeEnum.DELAYED_MODEL_OPTION:
    ...
    def processTickOptionComputationMsgProtoBuf(self, protobuf):
        ...
        tickType = (
            tickOptionComputationProto.tickType if tickOptionComputationProto.HasField("tickType") else UNSET_INTEGER
        )
        tickAttrib = (
            tickOptionComputationProto.tickAttrib
            if tickOptionComputationProto.HasField("tickAttrib")
            else UNSET_INTEGER
        )
        impliedVol = (
            tickOptionComputationProto.impliedVol if tickOptionComputationProto.HasField("impliedVol") else None
        )
        ...
        if optPrice is not None and optPrice == -1:  # -1 is the "not computed" indicator
            optPrice = None
        pvDividend = (
            tickOptionComputationProto.pvDividend if tickOptionComputationProto.HasField("pvDividend") else None
        )
    ...
    def processMarketDataTypeMsgProtoBuf(self, protobuf):
        ...
        marketDataType = (
            marketDataTypeProto.marketDataType if marketDataTypeProto.HasField("marketDataType") else UNSET_INTEGER
        )
    ...
    def processCommissionAndFeesReportMsgProtoBuf(self, protobuf):
        ...
        commissionAndFeesReport.execId = (
            commissionAndFeesReportProto.execId if commissionAndFeesReportProto.HasField("execId") else ""
        )
        commissionAndFeesReport.commissionAndFees = (
            commissionAndFeesReportProto.commissionAndFees
            if commissionAndFeesReportProto.HasField("commissionAndFees")
            else 0.0
        )
        commissionAndFeesReport.currency = (
            commissionAndFeesReportProto.currency if commissionAndFeesReportProto.HasField("currency") else ""
        )
        commissionAndFeesReport.realizedPNL = (
            commissionAndFeesReportProto.realizedPNL if commissionAndFeesReportProto.HasField("realizedPNL") else 0.0
        )
        commissionAndFeesReport.yield_ = (
            commissionAndFeesReportProto.bondYield if commissionAndFeesReportProto.HasField("bondYield") else 0.0
        )
        commissionAndFeesReport.yieldRedemptionDate = (
            int(commissionAndFeesReportProto.yieldRedemptionDate)
            if commissionAndFeesReportProto.HasField("yieldRedemptionDate")
            else 0
        )
    ...
    def processSecurityDefinitionOptionParameterMsgProtoBuf(self, protobuf):
        ...
        underlyingConId = (
            secDefOptParameterProto.underlyingConId if secDefOptParameterProto.HasField("underlyingConId") else 0
        )
    ...
    def processTickReqParamsMsgProtoBuf(self, protobuf):
        ...
        snapshotPermissions = (
            tickReqParamsProto.snapshotPermissions
            if tickReqParamsProto.HasField("snapshotPermissions")
            else UNSET_INTEGER
        )
    ...
    def processRerouteMktDepthReqMsgProtoBuf(self, protobuf):
        ...
        reqId = (
            rerouteMarketDepthRequestProto.reqId if rerouteMarketDepthRequestProto.HasField("reqId") else NO_VALID_ID
        )
        conId = rerouteMarketDepthRequestProto.conId if rerouteMarketDepthRequestProto.HasField("conId") else 0
        exchange = (
            rerouteMarketDepthRequestProto.exchange if rerouteMarketDepthRequestProto.HasField("exchange") else ""
        )
    ...
    def processTickByTickMsg(self, fields):
        ...
        if tickType == 0:
            ...
            self.wrapper.tickByTickBidAsk(reqId, time, bidPrice, askPrice, bidSize, askSize, tickAttribBidAsk)
        elif tickType == 4:
            # MidPoint
    ...
    def processHistoricalScheduleMsgProtoBuf(self, protobuf):
        ...
        startDateTime = (
            historicalScheduleProto.startDateTime if historicalScheduleProto.HasField("startDateTime") else ""
        )
        ...
        if historicalScheduleProto.historicalSessions:
            for historicalSessionProto in historicalScheduleProto.historicalSessions:
                ...
                historicalSession.startDateTime = (
                    historicalSessionProto.startDateTime if historicalSessionProto.HasField("startDateTime") else ""
                )
                historicalSession.endDateTime = (
                    historicalSessionProto.endDateTime if historicalSessionProto.HasField("endDateTime") else ""
                )
                historicalSession.refDate = (
                    historicalSessionProto.refDate if historicalSessionProto.HasField("refDate") else ""
                )
                sessions.append(historicalSession)
    ...
    def processCurrentTimeInMillisMsgProtoBuf(self, protobuf):
        ...
        timeInMillis = (
            currentTimeInMillisProto.currentTimeInMillis
            if currentTimeInMillisProto.HasField("currentTimeInMillis")
            else 0
        )
    ...
    def processErrorMsgProtoBuf(self, protobuf):
        ...
        advancedOrderRejectJson = (
            errorMessageProto.advancedOrderRejectJson if errorMessageProto.HasField("advancedOrderRejectJson") else ""
        )
    ...
    def processDisplayGroupUpdatedMsgProtoBuf(self, protobuf):
        ...
        contractInfo = (
            displayGroupUpdatedProto.contractInfo if displayGroupUpdatedProto.HasField("contractInfo") else ""
        )
    ...
    def interpret(self, fields, msgId):
        ...
        try:
            ...
            self.wrapper.error(NO_VALID_ID, currentTimeMillis(), BAD_MESSAGE.code(), BAD_MESSAGE.msg() + theBadMsg)
            raise
    ...
    msgId2handleInfoProtoBuf = {
        ...
        IN.SECURITY_DEFINITION_OPTION_PARAMETER_END: HandleInfo(
            proc=processSecurityDefinitionOptionParameterEndMsgProtoBuf
        ),
