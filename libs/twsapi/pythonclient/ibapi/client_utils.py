...
from ibapi.errors import ERROR_ENCODING_PROTOBUF
...
def createExecutionRequestProto(reqId: int, execFilter: ExecutionFilter) -> ExecutionRequestProto:
    ...
    if isValidIntValue(execFilter.clientId):
...
def createPlaceOrderRequestProto(orderId: int, contract: Contract, order: Order) -> PlaceOrderRequestProto:
    ...
    if attachedOrdersProto is not None:
        placeOrderRequestProto.attachedOrders.CopyFrom(attachedOrdersProto)
...
def createAttachedOrdersProto(order: Order) -> AttachedOrdersProto:
    ...
    if isValidIntValue(order.slOrderId):
        attachedOrdersProto.slOrderId = order.slOrderId
    if order.slOrderType:
        attachedOrdersProto.slOrderType = order.slOrderType
    if isValidIntValue(order.ptOrderId):
        attachedOrdersProto.ptOrderId = order.ptOrderId
...
def createContractProto(contract: Contract, order: Order) -> ContractProto:
    ...
    if isValidIntValue(contract.conId):
...
def createDeltaNeutralContractProto(contract: Contract) -> DeltaNeutralContractProto:
    ...
    if contract.deltaNeutralContract is not None:
        ...
        if isValidIntValue(deltaNeutralContract.conId):
            deltaNeutralContractProto.conId = deltaNeutralContract.conId
        if isValidFloatValue(deltaNeutralContract.delta):
            deltaNeutralContractProto.delta = deltaNeutralContract.delta
        if isValidFloatValue(deltaNeutralContract.price):
...
def createComboLegProto(comboLeg: ComboLeg, perLegPrice: float) -> ComboLegProto:
    ...
    if isValidIntValue(comboLeg.conId):
        comboLegProto.conId = comboLeg.conId
    if isValidIntValue(comboLeg.ratio):
...
def createOrderProto(order: Order) -> OrderProto:
    ...
    if isValidIntValue(order.clientId):
        orderProto.clientId = order.clientId
    if isValidLongValue(order.permId):
    ...
    if isValidFloatValue(order.lmtPrice):
        orderProto.lmtPrice = order.lmtPrice
    if isValidFloatValue(order.auxPrice):
    ...
    if isValidFloatValue(order.percentOffset):
        orderProto.percentOffset = order.percentOffset
    if isValidFloatValue(order.trailingPercent):
        orderProto.trailingPercent = order.trailingPercent
    if isValidFloatValue(order.trailStopPrice):
    ...
    if isValidFloatValue(order.volatility):
    ...
    if isValidFloatValue(order.deltaNeutralAuxPrice):
    ...
    if isValidFloatValue(order.scalePriceIncrement):
        orderProto.scalePriceIncrement = order.scalePriceIncrement
    if isValidFloatValue(order.scalePriceAdjustValue):
    ...
    if isValidFloatValue(order.scaleProfitOffset):
    ...
    if isValidIntValue(order.hedgeMaxSize):
        orderProto.hedgeMaxSize = order.hedgeMaxSize
    ...
    if isValidFloatValue(order.peggedChangeAmount):
    ...
    if isValidFloatValue(order.referenceChangeAmount):
    ...
    if isValidFloatValue(order.triggerPrice):
        orderProto.triggerPrice = order.triggerPrice
    if isValidFloatValue(order.adjustedStopPrice):
        orderProto.adjustedStopPrice = order.adjustedStopPrice
    if isValidFloatValue(order.adjustedStopLimitPrice):
        orderProto.adjustedStopLimitPrice = order.adjustedStopLimitPrice
    if isValidFloatValue(order.adjustedTrailingAmount):
    ...
    if isValidFloatValue(order.lmtPriceOffset):
    ...
    if isValidFloatValue(order.cashQty):
    ...
    if isValidFloatValue(order.competeAgainstBestOffset):
        orderProto.competeAgainstBestOffset = order.competeAgainstBestOffset
    if isValidFloatValue(order.midOffsetAtWhole):
        orderProto.midOffsetAtWhole = order.midOffsetAtWhole
    if isValidFloatValue(order.midOffsetAtHalf):
    ...
    if order.seekPriceImprovement is not None:
        orderProto.seekPriceImprovement = 1 if order.seekPriceImprovement else 0
    if isValidIntValue(order.whatIfType):
        orderProto.whatIfType = order.whatIfType
    if order.routeMarketableToBbo is not None:
        orderProto.routeMarketableToBbo = 1 if order.routeMarketableToBbo else 0
...
def createConditionsProto(order: Order) -> list[OrderConditionProto]:
    ...
    try:
        ...
        raise ClientException(
            ERROR_ENCODING_PROTOBUF.code(), ERROR_ENCODING_PROTOBUF.msg(), "Error encoding conditions"
        )
...
def createContractConditionProto(contractCondition: ContractCondition) -> OrderConditionProto:
    ...
    if isValidIntValue(contractCondition.conId):
        contractConditionProto.conId = contractCondition.conId
    if contractCondition.exchange:
        contractConditionProto.exchange = contractCondition.exchange
...
def createPriceConditionProto(priceCondition: PriceCondition) -> OrderConditionProto:
    ...
    if isValidFloatValue(priceCondition.price):
...
def createMarginConditionProto(marginCondition: MarginCondition) -> OrderConditionProto:
    ...
    if isValidFloatValue(marginCondition.percent):
...
def createExecutionConditionProto(executionCondition: ExecutionCondition) -> OrderConditionProto:
    ...
    if executionCondition.exchange:
        executionConditionProto.exchange = executionCondition.exchange
...
def createPercentChangeConditionProto(percentChangeCondition: PercentChangeCondition) -> OrderConditionProto:
    ...
    if isValidFloatValue(percentChangeCondition.changePercent):
...
def createContractDataRequestProto(reqId: int, contract: Contract) -> ContractDataRequestProto:
    ...
    if isValidIntValue(reqId):
        contractDataRequestProto.reqId = reqId
    contractProto = createContractProto(contract, None)
    if contractProto is not None:
        contractDataRequestProto.contract.CopyFrom(contractProto)
...
def createMarketDataRequestProto(
    ...
    marketDataOptionsList: TagValueList,
...
def createMarketDepthRequestProto(
    reqId: int, contract: Contract, numRows: int, isSmartDepth: bool, marketDepthOptionsList: TagValueList
) -> MarketDepthRequestProto:
    marketDepthRequestProto = MarketDepthRequestProto()
    if isValidIntValue(reqId):
        marketDepthRequestProto.reqId = reqId
    contractProto = createContractProto(contract, None)
    if contractProto is not None:
        marketDepthRequestProto.contract.CopyFrom(contractProto)
    if isValidIntValue(numRows):
        marketDepthRequestProto.numRows = numRows
    if isSmartDepth:
...
def createCancelMarketDepthProto(reqId: int, isSmartDepth: bool) -> CancelMarketDepthProto:
    ...
    if isValidIntValue(reqId):
        cancelMarketDepthProto.reqId = reqId
    if isSmartDepth:
...
def createAccountSummaryRequestProto(reqId: int, group: str, tags: str) -> AccountSummaryRequestProto:
    ...
    if isValidIntValue(reqId):
        accountSummaryRequestProto.reqId = reqId
    if group:
...
def createCancelAccountSummaryRequestProto(reqId: int) -> CancelAccountSummaryProto:
    ...
    if isValidIntValue(reqId):
        cancelAccountSummaryProto.reqId = reqId
...
def createRealTimeBarsRequestProto(
    reqId: int, contract: Contract, barSize: int, whatToShow: str, useRTH: bool, realTimeBarsOptionsList: TagValueList
...
def createCancelHeadTimestampProto(reqId: int) -> CancelHeadTimestampProto:
    ...
    if isValidIntValue(reqId):
        cancelHeadTimestampProto.reqId = reqId
...
def createCancelHistogramDataProto(reqId: int) -> CancelHistogramDataProto:
    ...
    if isValidIntValue(reqId):
        cancelHistogramDataProto.reqId = reqId
...
def createNewsArticleRequestProto(
    reqId: int, providerCode: str, articleId: str, newsArticleOptionsList: TagValueList
...
def createHistoricalNewsRequestProto(
    ...
    endDateTime: str,
    totalResults: int,
    historicalNewsOptionsList: TagValueList,
...
def createWshMetaDataRequestProto(reqId: int) -> WshMetaDataRequestProto:
    ...
    if isValidIntValue(reqId):
        wshMetaDataRequestProto.reqId = reqId
...
def createCancelWshMetaDataProto(reqId: int) -> CancelWshMetaDataProto:
    ...
    if isValidIntValue(reqId):
        cancelWshMetaDataProto.reqId = reqId
...
def createWshEventDataRequestProto(reqId: int, wshEventData: WshEventData) -> WshEventDataRequestProto:
    ...
    if isValidIntValue(reqId):
        wshEventDataRequestProto.reqId = reqId

    if wshEventData is not None:
        if isValidIntValue(wshEventData.conId):
            wshEventDataRequestProto.conId = wshEventData.conId
        if wshEventData.filter:
        ...
        if wshEventData.startDate:
            wshEventDataRequestProto.startDate = wshEventData.startDate
        if wshEventData.endDate:
            wshEventDataRequestProto.endDate = wshEventData.endDate
        if isValidIntValue(wshEventData.totalLimit):
            wshEventDataRequestProto.totalLimit = wshEventData.totalLimit
...
def createCancelWshEventDataProto(reqId: int) -> CancelWshEventDataProto:
    ...
    if isValidIntValue(reqId):
        cancelWshEventDataProto.reqId = reqId
...
def createScannerSubscriptionRequestProto(
    ...
    scannerSubscriptionOptionsList: TagValueList,
    scannerSubscriptionFilterOptionsList: TagValueList,
    ...
    if scannerSubscriptionProto is not None:
        scannerSubscriptionRequestProto.scannerSubscription.CopyFrom(scannerSubscriptionProto)
...
def createScannerSubscriptionProto(
    ...
    if isValidFloatValue(subscription.abovePrice):
        scannerSubscriptionProto.abovePrice = subscription.abovePrice
    if isValidFloatValue(subscription.belowPrice):
    ...
    if isValidIntValue(subscription.averageOptionVolumeAbove):
        scannerSubscriptionProto.averageOptionVolumeAbove = subscription.averageOptionVolumeAbove
    if isValidFloatValue(subscription.marketCapAbove):
        scannerSubscriptionProto.marketCapAbove = subscription.marketCapAbove
    if isValidFloatValue(subscription.marketCapBelow):
    ...
    if isValidFloatValue(subscription.couponRateAbove):
        scannerSubscriptionProto.couponRateAbove = subscription.couponRateAbove
    if isValidFloatValue(subscription.couponRateBelow):
...
def createFundamentalsDataRequestProto(
    reqId: int, contract: Contract, reportType: str, fundamentalsDataOptionsList: TagValueList
...
def createCancelFundamentalsDataProto(reqId: int) -> CancelFundamentalsDataProto:
    ...
    if isValidIntValue(reqId):
        cancelFundamentalsDataProto.reqId = reqId
...
def createCalculateImpliedVolatilityRequestProto(
    reqId: int, contract: Contract, optionPrice: float, underPrice: float, impliedVolatilityOptionsList: TagValueList
    ...
    if isValidFloatValue(optionPrice):
        calculateImpliedVolatilityRequestProto.optionPrice = optionPrice
    if isValidFloatValue(underPrice):
...
def createCalculateOptionPriceRequestProto(
    reqId: int, contract: Contract, volatility: float, underPrice: float, optionPriceOptionsList: TagValueList
    ...
    if isValidFloatValue(volatility):
...
def createSecDefOptParamsRequestProto(
    reqId: int, underlyingSymbol: str, futFopExchange: str, underlyingSecType: str, underlyingConId: int
    ...
    if isValidIntValue(underlyingConId):
...
def createMarketRuleRequestProto(marketRuleId: int) -> MarketRuleRequestProto:
    ...
    if isValidIntValue(marketRuleId):
...
def createUserInfoRequestProto(reqId: int) -> UserInfoRequestProto:
    ...
    if isValidIntValue(reqId):
        userInfoRequestProto.reqId = reqId
...
def createIdsRequestProto(numIds: int) -> IdsRequestProto:
    ...
    if isValidIntValue(numIds):
        idsRequestProto.numIds = numIds
...
def createStartApiRequestProto(clientId: int, optionalCapabilities: str) -> StartApiRequestProto:
    ...
    if isValidIntValue(clientId):
        startApiRequestProto.clientId = clientId
    if optionalCapabilities:
        startApiRequestProto.optionalCapabilities = optionalCapabilities
...
def createVerifyRequestProto(apiName: str, apiVersion: str) -> VerifyRequestProto:
    ...
    if apiName:
        verifyRequestProto.apiName = apiName
    if apiVersion:
        verifyRequestProto.apiVersion = apiVersion
...
def createVerifyMessageRequestProto(apiData: str) -> VerifyMessageRequestProto:
    ...
    if apiData:
        verifyMessageRequestProto.apiData = apiData
...
def createQueryDisplayGroupsRequestProto(reqId: int) -> QueryDisplayGroupsRequestProto:
    ...
    if isValidIntValue(reqId):
        queryDisplayGroupsRequestProto.reqId = reqId
...
def createSubscribeToGroupEventsRequestProto(reqId: int, groupId: int) -> SubscribeToGroupEventsRequestProto:
    ...
    if isValidIntValue(reqId):
        subscribeToGroupEventsRequestProto.reqId = reqId
    if isValidIntValue(groupId):
        subscribeToGroupEventsRequestProto.groupId = groupId
...
def createUpdateDisplayGroupRequestProto(reqId: int, contractInfo: str) -> UpdateDisplayGroupRequestProto:
    ...
    if isValidIntValue(reqId):
        updateDisplayGroupRequestProto.reqId = reqId
    if contractInfo:
        updateDisplayGroupRequestProto.contractInfo = contractInfo
...
def createUnsubscribeFromGroupEventsRequestProto(reqId: int) -> UnsubscribeFromGroupEventsRequestProto:
    ...
    if isValidIntValue(reqId):
        unsubscribeFromGroupEventsRequestProto.reqId = reqId
...
def createCancelContractDataProto(reqId: int) -> CancelContractDataProto:
    ...
    if isValidIntValue(reqId):
        cancelContractDataProto.reqId = reqId
...
def createCancelHistoricalTicksProto(reqId: int) -> CancelHistoricalTicksProto:
    ...
    if isValidIntValue(reqId):
        cancelHistoricalTicksProto.reqId = reqId
