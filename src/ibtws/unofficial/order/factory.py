...
def bracket_to_orders(request: BracketRequest, parent_ref: str, tp_ref: str, sl_ref: str) -> list[Order]:
    """Build a parent + take-profit + stop-loss triplet, OCA-grouped via shared ``orderRef`` prefix.

    Mirrors ``ib_async.IB.bracketOrder`` but lets us own the orderId / orderRef
    wiring so the reconciler can match all three legs back to the same group.
    """
